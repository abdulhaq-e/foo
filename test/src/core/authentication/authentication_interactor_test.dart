...
import 'package:foo/core-testing.dart';
...
void main() {
  group('AuthenticationInteractor', () {
      ...
      test('equality guard prevents duplicate event on refresh', () async {
        ...
        expect(states, [
          ...
          AuthenticationStatus
              .authenticated, // Only ONE authenticated event from refresh
        ]);
    ...
    group('Equality Guard Edge Case', () {
      ...
      test('handles logout from another tab (store emits null)', () async {
        const authResponse = AuthenticationResponse(
          token: 'token-ABC',
          saasTenantEntityId: 'tenant-123',
        );
        ...
        expect(states, [
          AuthenticationStatus.authenticated,
          AuthenticationStatus
              .unauthenticated, // Reacts to logout from another tab
        ]);
