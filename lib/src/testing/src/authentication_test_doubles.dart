...
class FakeAuthenticationService with AuthenticationService {
  final _asyncAuthController = StreamController<AuthenticationEvent>.broadcast();

  AuthenticationResponse? _lastAuthResponse;
  bool _isAuthenticated = false;

  @override
  Stream<AuthenticationEvent> get asyncAuthentication => _asyncAuthController.stream;
  ...
  void emitFailedAuth([Object? error]) {
    _isAuthenticated = false;
    _asyncAuthController.add(
      FailedAuthenticationEvent(error: error),
    );
  }
...
class FakeAuthenticationStore extends AuthenticationStore {
  final _authStateController = StreamController<AuthenticationResponse?>.broadcast();
